...
float sdLine(vec2 p, float r) {
  float halfLen = r * 2.0;
  vec2 a = vec2(-halfLen, 0.0); vec2 b = vec2(halfLen, 0.0);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  float t = iTime * 0.5; vec2 pos = uv - 0.5;
...
function AuraShader({
  shape = 1.0, speed = 1.0, amplitude = 0.5, frequency = 0.5, scale = 0.2,
  blur = 1.0, color = DEFAULT_COLOR, colorShift = 1.0, brightness = 1.0,
    ...
    <div ref={ref} className={className} {...props}>
      <ReactShaderToy
        ...
        uniforms={{
          ...
          uColor: { type: '3fv', value: rgbColor ?? [0.86, 0.15, 0.28] },
        }}
...
export function AgentAudioVisualizerAura({
  size = 'lg', state = 'connecting', color = DEFAULT_COLOR, colorShift = 0.05,
  audioTrack, themeMode, className, ref, ...props
  ...
  return (
    <AuraShader
      ref={ref} data-lk-state={state} blur={0.2} color={color} colorShift={colorShift}
      speed={speed} scale={scale} themeMode={themeMode} amplitude={amplitude}
      frequency={frequency} brightness={brightness}
