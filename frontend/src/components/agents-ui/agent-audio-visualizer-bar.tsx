...
import React, { type CSSProperties, Children, type ComponentProps, type ReactNode, cloneElement, isValidElement, useMemo } from 'react';
  ...
  return Children.map(children, (child) => {
    if (isValidElement(child) && Children.only(children)) {
      ...
      if (childProps.className) {
        ...
        props.style = { ...(childProps.style as CSSProperties), ...(props.style as CSSProperties) };
      }
...
export const AgentAudioVisualizerBarElementVariants = cva(
  ...
  { variants: { size: { icon: 'w-[4px] min-h-[4px]', sm: 'w-[8px] min-h-[8px]', md: 'w-[16px] min-h-[16px]', lg: 'w-[32px] min-h-[32px]', xl: 'w-[64px] min-h-[64px]' } }, defaultVariants: { size: 'md' } },
);

export const AgentAudioVisualizerBarVariants = cva('relative flex items-center justify-center', {
  variants: { size: { icon: 'h-[24px] gap-[2px]', sm: 'h-[56px] gap-[4px]', md: 'h-[112px] gap-[8px]', lg: 'h-[224px] gap-[16px]', xl: 'h-[448px] gap-[32px]' } },
...
export function AgentAudioVisualizerBar({
  ...
  const _barCount = useMemo(() => {
    ...
    switch (size) { case 'icon': case 'sm': return 3; default: return 5; }
  }, [barCount, size]);
  ...
  if (children && Array.isArray(children)) throw new Error('AgentAudioVisualizerBar children must be a single element.');

  return (
    <div data-lk-state={state} style={{ ...style, color } as CSSProperties} className={cn(AgentAudioVisualizerBarVariants({ size }), className)} {...props}>
      {bands.map((band: number, idx: number) =>
        children ? (
          <React.Fragment key={idx}>
            {cloneSingleChild(children, { 'data-lk-index': idx, 'data-lk-highlighted': highlightedIndices.includes(idx), style: { height: `${band * 100}%` } })}
          </React.Fragment>
        ) : (
          <div key={idx} data-lk-index={idx} data-lk-highlighted={highlightedIndices.includes(idx)} style={{ height: `${band * 100}%` }} className={cn(AgentAudioVisualizerBarElementVariants({ size }))} />
        ),
      )}
    </div>
  );
