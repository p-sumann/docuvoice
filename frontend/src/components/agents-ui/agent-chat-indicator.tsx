...
const motionAnimationProps = {
  variants: {
    hidden: { opacity: 0, scale: 0.1, transition: { duration: 0.1, ease: 'linear' as const } },
    visible: {
      opacity: [0.5, 1],
      scale: [1, 1.2],
      transition: { type: 'spring' as const, bounce: 0, duration: 0.5, repeat: Infinity, repeatType: 'mirror' as const },
    },
  },
  initial: 'hidden',
  animate: 'visible',
...
const agentChatIndicatorVariants = cva('bg-muted-foreground inline-block size-2.5 rounded-full', {
  variants: { size: { sm: 'size-2.5', md: 'size-4', lg: 'size-6' } },
